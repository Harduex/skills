...
**Scripts stay in the skill; only state goes in the repo.** The three helper scripts ship *inside this skill* under `scripts/`. Invoke each by its path under this skill's base directory — written `$SKILL/scripts/…` below, where `$SKILL` is the directory the harness prints when the skill loads — with your working directory at the repo root. They write only to `docs/loop/<slug>/` and the ignored `.loop/<slug>/`, and nothing at all when bound to a substrate the repo already owns. **Never copy them into the project**: the containment boundary is the whole point — the tooling lives in the skill, and only the *state* is committed to the repo's `docs/`.
...
4. **State.** *Native:* scaffold `docs/loop/<slug>/` deterministically with `$SKILL/scripts/init.sh <slug>` — a **continuation note** (`CHECKPOINT.md`: status, the one current task, orientation, last verification, locked decisions, blockers, next action), `contract.md`, the first **task slice** (`NNN-*.md`), and a git-ignored journal under `.loop/<slug>/` (schemas in [REFERENCE.md](REFERENCE.md)). Fill the slices via your planning capability: one file per session-sized task, each executable by an agent with zero prior context, ordered so dependencies flow forward. A completed slice is **deleted in its own completion commit**, so the directory holds only current and future work while git history keeps every instruction ever executed. *Native, and required:* make the loop **discoverable** — the repo's agent instruction file must send a new session to the note as its first read. Add or repoint exactly one line there (never a second target — L10), because a state file nobody is told to open resumes nothing. No instruction file at all → create a minimal one naming the note. *Adopted:* the files already exist and the instruction file already points at them — write no parallel ones and change no pointer; fill only what the substrate leaves undeclared (usually the gate commands) in its own idiom. Either way, use your checkpoint/handoff capability to write the note — file-backed, not as a paste-in prompt.

## One cycle — repeat until the contract is green

1. **Read state.** `$SKILL/scripts/status.sh <slug>` (or `--adopt`), then re-read the note, the slice it names, and the journal tail from disk. Trust the files, not memory or session-start context. Uncommitted changes with the pointer still on a slice means the last session stopped mid-task — reconcile against that slice instead of advancing.
2. **Pick** the task the note names — one at a time, and it stays current until it is verified. A session-sized slice holds more than one cycle: work its own checklist as the sub-ledger, and never mint a new unit of work to track your cycles.
...
4. **Commit.** One local commit per task, and it is **atomic**: implementation, tests, the advanced continuation note, and `git rm` of the completed slice, together. Never a second "state commit" — a stop between the two leaves the pointer lying about what is done. Reversible, so allowed without asking. A substrate that declares different granularity overrides this.
...
6. **Record.** Append the measured result to the journal. Fold only the durable part into the note — what was verified, what got decided, what to do next — and retire the slice as part of the completion commit above. An unverified task keeps its slice and its pointer.
...
8. **Gate.** Contract fully green → stop, summarize, refresh the orientation for handoff, then **close the run out**: graduate every durable decision and unfinished human-verified item out of the note, retire the whole state directory in one commit, and repoint or remove the instruction-file line ([REFERENCE.md](REFERENCE.md) → *Closing out a run*). A finished run left in the tree becomes a second advertised note (L10). Otherwise → next cycle. A **judgment gate** is separate from both: a decision that is cheap and reversible but not yours to make (which prototype wins, which wording ships) is not a blocked task — set the note to `waiting-human` with the exact decision or evidence needed, commit the evidence *without* retiring the slice, leave the pointer where it is, and hand it over. A substrate's own gate state, where it has one, wins.
