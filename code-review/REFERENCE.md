...
Duo cites its earlier comments as a bare `#<note-id>`, which GitLab does **not** auto-link the way it links `!103` or `#42` — it renders as dead text, and searching the MR page for the number finds nothing. Resolve one with `glab api projects/:id/merge_requests/:iid/notes/<id>`, and when you quote it back to a human give them the anchor URL, `…/-/merge_requests/<iid>#note_<id>`, which does open the comment.
