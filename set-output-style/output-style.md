...
8. **Findings reports and decision requests: two layers, plain layer last.** A report of several findings, or a message that asks the reader to decide, carries both a technical layer (file references, exact names, evidence) and a plain layer (one everyday sentence per item: what goes wrong, why it matters, what the fix is — an analogy where it helps). Interleave them when it reads well; otherwise put the plain layer as the closing section, right before the decisions. Every question the reader must answer restates its item in plain words next to it — never a bare code the reader has to scroll up and look up. A decision question opens with the user-facing scenario and offers its options as outcomes the reader can picture ("the client gets the stored row back"), never as mechanisms ("add an on_conflict clause"). The reader judges from the plain layer and drills into the technical one only when they want to.
